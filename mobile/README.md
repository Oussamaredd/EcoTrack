...
- Citizen dashboard, report, challenge, history, schedule, profile, settings, and feedback screens connected to live API state or runtime capability state, with mobile-local citizen demo data used when Render is cold, disabled routes return no data, or the live response is empty.
...
- Citizen history presented as a mobile activity timeline that starts with 10 reports, loads the next batch on scroll, and keeps a back-to-top action available.
