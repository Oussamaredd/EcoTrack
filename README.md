...
- Workspace-wide transitive dependency pins that protect CI image scans live in the root `package.json` `overrides` block and the committed root `package-lock.json`; update those two files together so `npm ci` and Trivy scan the same resolved graph.
