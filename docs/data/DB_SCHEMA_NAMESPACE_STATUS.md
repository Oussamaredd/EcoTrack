...
- The seed requires `ALLOW_SOUTENANCE_SEED=true`, requires a dev-only `SOUTENANCE_DEMO_AUTH_HASH`, refuses `NODE_ENV=production`, and uses `SOUTENANCE_DATABASE_URL` or `DATABASE_POOLER_URL` only when the URL points to a Supabase Session pooler host on port `5432`.
