...
- `/app` must use the shared role hub panel and select citizen-specific onboarding copy for citizen sessions.
- The role hub must keep the citizen reporting action visible in its priority actions.
- That reporting action must route into the existing web report surface at `/app/citizen/report`.
...
- Non-citizen roles must use the same shared `/app` panel design with role-specific copy.
- Multi-role users may still see their other role lanes, but the panel copy is selected from the logged session role before falling back to access-based role priority.
...
- shared EcoTrack loading treatment while mapped-container and zone context is loading
...
- `/app` should fall back to a safe citizen entry treatment that still keeps `Citizen Reporting` available
...
| reporting workspace preparing | show the shared `Loading EcoTrack` state after the citizen opens `/app/citizen/report`; do not present this as forbidden access or render the report form before mapped-container context is ready |
...
- `/app` must keep using the shared role hub panel
- `/app` should keep the citizen lane lightweight for returning users
