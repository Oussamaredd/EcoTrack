...
| `/app/agent/tour` | `AgentTourPage` | Requires `agent`/`admin`/`super_admin`; otherwise shows Access Denied. Refresh reloads server tour state, while persisted-route rebuild remains a manager-only action. The page is zone-assigned, shows the zone depot/start location, loads all paginated mapped containers for that assigned zone, and renders them with the same operational marker family while numbered route badges stay reserved for the current routed stops. When the page is showing an overdue or cached tour snapshot, it also offers `Reload Without Cache` recovery. |
