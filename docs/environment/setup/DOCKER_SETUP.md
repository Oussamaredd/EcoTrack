...
- The API production-dependency stage also pins fixed transitive Node packages used by the runtime image; when the CI Trivy gate reports fixed `HIGH` or `CRITICAL` Node vulnerabilities, update `package.json`, `package-lock.json`, and the production image pins together.
