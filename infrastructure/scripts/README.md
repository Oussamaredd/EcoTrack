...
- `infrastructure/scripts/validate-mobile-readiness.mjs` verifies the Development mobile release lane and writes evidence under `tmp/quality/mobile-readiness` or `tmp/ci/quality/mobile-readiness`.
...
- `infrastructure/scripts/ci/generate-quality-scorecard.mjs` writes the Development-only release scorecard under `tmp/ci/release`.
...
- `infrastructure/scripts/ci/run-lighthouse-gate.mjs` builds the app, serves a local preview, injects mock browser auth env plus a lightweight mock API for auth-status and frontend-telemetry endpoints, and writes Lighthouse reports to `tmp/quality/lighthouse` or `tmp/ci/quality/lighthouse`.
