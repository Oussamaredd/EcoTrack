...
- `infrastructure/scripts/ci/run-lighthouse-gate.mjs` builds the app, serves a local preview, boots a lightweight mock API for auth-status and frontend-telemetry endpoints, and writes Lighthouse reports to `tmp/quality/lighthouse` or `tmp/ci/quality/lighthouse`.
